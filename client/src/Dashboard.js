    ...
    <div >
        ...
        { 
           course?.map((course)=>{
            ...
            return(
                <Link to={`/course/${course?._id}`} className='flex flex-col pb-5 justify-between backdrop-blur-md bg-white/20 rounded-3xl'><div><img alt={course.Title} src={course.Poster} className='h-24 shadow-md hover:shadow-border hover:size-2xl xs:h-48 w-40 xs:w-96 rounded-lg ' loading='lazy'/>
