...
//       const response = await axios.post(`${process.env.REACT_APP_URL}/users/signup`, value);
//       navigate(`/verifyEmail/${response.data.newUser._id}`);
//     } catch (error) {
//       console.error(error);
//       setErrored([true, 'Email already exists. Please try again!']);
...
//   return (
//     <div className='pt-20 h-screen bg-stone-100'>
//       <form className="max-w-sm mx-auto border mt-10 text-white bg-white p-10  rounded-3xl shadow-lg" onSubmit={handleSubmit(submitForm)}>
//         <h1 className='text-green-700 pb-3 font-bold'>Sign Up</h1>
//         <div className='flex flex-col py-1'>
//           <p className='text-red-950'>{errored[1]}</p>
...
//         <div className='flex flex-col py-1'>
//           <div className='border bg-white border-border flex justify-between items-center   rounded-3xl'>
//             <input type={type} className=' border-white focus:border-0 text-black pl-2 h-10 w-full  focus:outline-0 ring-0 rounded-3xl ' placeholder='Password' {...register("password")} />
//             <button onClick={(e) => toggleVisibility(e, type, setType)} className='pr-2 mr-1 bg-white text-black pl-2'><FaRegEye /></button>
//           </div>
//           <p className='text-red-400 text-sm'>{errors.password?.message}</p>
//         </div>
//         <div className='flex flex-col py-1'>
//           <div className='border border-border flex  bg-white justify-between items-center   rounded-3xl'>
//             <input type={types} className=' border-white text-black focus:border-0  pl-2 h-10 w-full   rounded-3xl ' placeholder='Confirm Password' {...register("passwordConfirm")} />
//             <button onClick={(e) => toggleVisibility(e, types, setTypes)} className='pr-2 pl-2 mr-1 bg-white   text-black'><FaRegEye /></button>
//           </div>
//           <p className='text-red-400 text-sm'>{errors.passwordConfirm?.message}</p>
//         </div>
//         {loading ? (
//           <button type='submit' disabled className="w-full mt-3 h-9 rounded-3xl p-1 text-white flex gap-2 items-center justify-center bg-purple-700 text-sm">
//             <LoadingOutlined spinning allowFullScreen size="large" style={{ color: "white" }} /> Sign Up
//           </button>
//         ) : (
//           <button type='submit' className="w-full mt-3 h-9 rounded-3xl p-1 text-white flex items-center hover:bg-white hover:text-purple-700 hover:border justify-center bg-purple-700 text-sm">
//             Sign Up
//           </button>
//         )}
//         <p className='text-slate-500 text-sm pt-1 pl-1'>By clicking sign up you agree to our user <em className='underline underline-offset-2'>Terms of Service</em> and <em className='underline underline-offset-2'>Privacy Policy</em></p>
  ...
  return (
    <div className='min-h-screen bg-gray-900 flex items-center justify-center'>
      <div className='bg-gray-800 p-8 rounded-lg shadow-lg max-w-sm w-full'>
        <h1 className='text-center text-2xl font-bold text-purple-400 mb-6'>Sign Up</h1>
        {errored[0] && <p className='text-red-500 text-sm mb-4'>{errored[1]}</p>}
        <form onSubmit={handleSubmit(submitForm)}>
          <div className='mb-4'>
            <input
              type='text'
              className='w-full px-3 py-2 rounded-lg border border-gray-700 focus:outline-none focus:border-purple-400'
          ...
          <div className='mb-4'>
            <div className='relative'>
              <input
                type={type}
                className='w-full px-3 py-2 rounded-lg border border-gray-700 focus:outline-none focus:border-purple-400 pr-10'
          ...
          <button
            type='submit'
            className={`w-full py-2 rounded-lg text-white bg-purple-700 hover:bg-purple-600 focus:outline-none ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
            disabled={loading}
          >
            {loading ? (
        ...
        <p className='text-center text-sm text-gray-400 mt-4'>
          By clicking sign up you agree to our user <em className='text-purple-400 cursor-pointer'>Terms of Service</em> and <em className='text-purple-400 cursor-pointer'>Privacy Policy</em>
        </p>
      </div>
    </div>
  );
